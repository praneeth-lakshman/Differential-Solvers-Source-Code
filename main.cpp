...
class RKF {
 public:
  ...
  Values next_step(double y, double h, py::function f, double t) {
    ...
    double k4 =
        h * f(y + a41 * k1 + a42 * k2 + a43 * k3, t + h * c4).cast<double>();
...
class DormandPrince {
 public:
  ...
  const double a51 = 19372.0 / 6561, a52 = -25360.0 / 2187,
               a53 = 64448.0 / 6561, a54 = -212.0 / 729;
  const double a61 = 9017.0 / 3168, a62 = -355.0 / 33, a63 = 46732.0 / 5247,
               a64 = 49.0 / 176, a65 = -5103.0 / 18656;
  const double a71 = 35.0 / 384, a72 = 0.0, a73 = 500.0 / 1113,
               a74 = 125.0 / 192, a75 = -2187.0 / 6784, a76 = 11.0 / 84;
  ...
  double eps;
  DormandPrince(double e = 0.005) { eps = e; }
  Values next_step(double y, double h, py::function f, double t) {
    double k1 = h * f(y, t + h * c1).cast<double>();
    double k2 = h * f(y + a21 * k1, t + h * c2).cast<double>();
    double k3 = h * f(y + a31 * k1 + a32 * k2, t + h * c3).cast<double>();
    ...
    double k6 = h * f(y + a61 * k1 + a62 * k2 + a63 * k3 + a64 * k4 + a65 * k5,
                      t + h * c6)
                        .cast<double>();
    double k7 = h * f(y + a71 * k1 + a72 * k2 + a73 * k3 + a74 * k4 + a75 * k5 +
                          a76 * k6,
                      t + h * c7)
                        .cast<double>();
...
PYBIND11_MODULE(ODESolvers, handle) {
  ...
  py::class_<Euler>(handle, "Euler")
      .def(py::init<>())
      .def("next_step", &Euler::next_step);
  py::class_<RK4>(handle, "RK4")
      .def(py::init<>())
      .def("next_step", &RK4::next_step);
  ...
  py::class_<DormandPrince::Values>(handle, "DormandPrinceValues")
      .def_readwrite("y", &DormandPrince::Values::y)
      .def_readwrite("h", &DormandPrince::Values::h)
      .def_readwrite("accepted", &DormandPrince::Values::accepted);
